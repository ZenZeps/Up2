...
export default function Invite() {
    ...
    useEffect(() => {
        if (Platform.OS !== 'web') {
            if (eventId) {
                router.replace(`/(root)/events/${eventId}` as any);
            } else {
    ...
    const handleAcceptInvite = async () => {
        ...
        try {
            ...
            router.push(`/(root)/events/${event.$id}`);
        } catch (error) {
    ...
    if (Platform.OS !== 'web') {
        // On native, show a loading spinner while redirecting.
        return (
            <SafeAreaView className="flex-1 bg-white">
                <View className="flex-1 justify-center items-center">
                    <ActivityIndicator size="large" color="#0061FF" />
                    <Text className="mt-4 text-gray-500 text-lg">Loading Event...</Text>
                </View>
            </SafeAreaView>
        );
    }
