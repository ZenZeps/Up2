...
const Settings = () => {
    ...
    const handleLogout = async () => {
        try {
            // Use the proper logout function that clears cache
            const { logout } = await import('@/lib/appwrite/appwrite');
            await logout();

            // Clear AsyncStorage data that might persist user info
            try {
                const AsyncStorage = (await import('@react-native-async-storage/async-storage')).default;

                // Get all keys and remove user-specific data
                const allKeys = await AsyncStorage.getAllKeys();
                const userDataKeys = allKeys.filter(key =>
                    key.startsWith('@AppCache:') ||
                    key.startsWith('@ScreenCache:') ||
                    key.startsWith('@DataFetch:') ||
                    key.includes('user-') ||
                ...
                if (userDataKeys.length > 0) {
                    ...
                    console.log(`Cleared ${userDataKeys.length} cached items from storage`);
                }
            } catch (storageError) {
                console.warn('Could not clear AsyncStorage (non-critical):', storageError);
            }

            // Refresh global context to clear user state
            await refetch();
            ...
            // Fallback: still try to clear session and navigate
            try {
    ...
    return (
        <SafeAreaView style={[styles.container, { backgroundColor: colors.background }]}>
            ...
            <View style={[styles.header, { backgroundColor: colors.card, borderBottomColor: colors.border }]}>
                <TouchableOpacity style={styles.backButton} onPress={() => router.push('/(root)/(tabs)/Profile')}>
