 ...
 * Preloads essential data during splash screen to improve app startup performance
 */
    ...
    private async performPreload(userId?: string, skipCache = false): Promise<void> {
        try {
            ...
            await this.preloadPublicEvents(userId, skipCache)
                .catch(error => console.warn('Failed to preload events:', error));
            ...
            if (userId) {
                ...
                const userDataTasks = [
                    this.preloadUserData(userId, skipCache)
                        .catch(error => console.warn('Failed to preload user data:', error)),
                    this.preloadMiscData()
                        .catch(error => console.warn('Failed to preload misc data:', error))
                ];
        ...
        try {
            ...
            if (!skipCache) {
                ...
                if (cachedEvents && Array.isArray(cachedEvents) && cachedEvents.length > 0) {
                    console.log('📦 Using cached events for preload - Home/Feed ready instantly');
                    return;
                }
            }

            console.log('🎉 Preloading public events for Home and Feed...');
            ...
            console.warn('❌ Failed to preload public events:', error);
